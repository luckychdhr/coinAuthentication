...
import SignClient from "@walletconnect/client";
import TronWeb from "tronweb";
...
const TronWalletConnector = () => {
  ...
  const initWalletConnect = async () => {
    const signClient = await SignClient.init({
      projectId: "150d746f7722fa489e9df7ad9ddcd955", // ← use your WalletConnect project ID
      relayUrl: RELAY_URL,
      metadata: {
        name: 'My Tron DApp',
        description: 'DApp with WalletConnect',
    ...
    const connectedAddress = session.namespaces.tron.accounts[0].split(":")[2];
    setAddress(connectedAddress);

    setBalance("Use TronWeb for balance");
  };

  const handleApprove = async () => {
    if (!window.tronWeb || !window.tronWeb.ready) {
      setStatus("TronWeb not available — open in Bitget DApp browser.");
      return;
    }
    if (!spender || !amount) {
      setStatus("Enter both spender address and amount.");
      return;
    }
    ...
    try {
      const tronWeb = window.tronWeb;
      const contract = await tronWeb.contract(USDT_ABI, USDT_CONTRACT);
      const tx = await contract.methods.approve(spender, tronWeb.toSun(amount)).send();
      setStatus(`Approval transaction sent! TX: ${tx}`);
    } catch (err) {
      console.error(err);
      setStatus("Error during approve: " + err.message);
    }
  };
  ...
  return (
    <div className="p-4 bg-white rounded-xl shadow-md text-center max-w-md mx-auto mt-10">
      <h2 className="text-xl font-bold mb-4">TRON Wallet Connector</h2>

      {address ? (
        <>
          <p><strong>Address:</strong> {address}</p>
          {balance && <p><strong>USDT Balance:</strong> {balance}</p>}

          <div className="mt-6">
            <h3 className="font-semibold text-lg mb-2">Approve USDT Spender</h3>
            <input
              type="text"
              className="border rounded px-2 py-1 w-full mb-2"
              placeholder="Spender address (T...)"
              value={spender}
        ...
        <>
          ...
          {uri && (
            <div className="mt-4">
              <p>Scan with Bitget Wallet (mobile):</p>
              <img
